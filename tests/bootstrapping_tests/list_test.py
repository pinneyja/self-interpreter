...
def test_list_copy_size(interpreter):
	parser = Parser()
...
def test_list_adding_accessing_simple(interpreter):
	parser = Parser()
...
def test_list_add_all(interpreter): 
	parser = Parser()
	interpreter.interpret(parser.parse("_AddSlots: (| test_list = list copy|)"))
...
def test_list_first_link_satisfying(interpreter): 
	parser = Parser()
	interpreter.interpret(parser.parse("_AddSlots: (| test_list = list copy|)"))
	interpreter.interpret(parser.parse("((((test_list add: 0) add: 1) add: 2) add: 3) add: 4"))
...
def test_list_iteration(interpreter): 
	parser = Parser()
...
def test_list_insertion_removal(interpreter):
	parser = Parser()
	interpreter.interpret(parser.parse("_AddSlots: (| test_list = list copy|)"))
	interpreter.interpret(parser.parse("((((test_list add: 0) add: 1) add: 2) add: 3) add: 4"))
